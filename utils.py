...
def leaky_relu_function(vector):
...
def leaky_relu_function_derivative(vector):
...
def correct_weights(matrix_1, matrix_2,hidden, dX, alpha, input_matrix=[], Y=[], S=[], layer=1, vector=0):
    ...
    if layer == 1:
        ...
        save_matrix = multiplication(save_matrix, [leaky_relu_function_derivative(S[0])])
        ...
        save_matrix = multiplication(transpose(hidden), [leaky_relu_function_derivative(Y[0])])
...
def step_training(i, input_matrix, context_matrix_elman, alpha, matrix_1, matrix_2):
    ...
    hidden = [leaky_relu_function(S[0])]
    Y = multiplication(hidden, matrix_2)

    output = [leaky_relu_function(Y[0])]
